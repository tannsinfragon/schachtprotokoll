...
1. **Versionsnummer an vier Stellen synchron halten:** `assets/js/app-config.js` (`version`), Fallback in `script.js`, `CACHE_NAME`/`?v=`-Query-Strings in `serviceworker.js`, und die `?v=`-Query-Strings der `<script>`-Tags in `index.html` (sonst fordert die Seite eine andere Query-String-Version an, als der Service Worker vorgecacht hat). Aktuell: `2.8.10` überall, README ebenfalls synchron.
